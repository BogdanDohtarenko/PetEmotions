...
object DayAttributeMapper {
    fun dbModelToEntity(dbModel: DayAttributesDbModel):DayAttribute {
        return DayAttribute(id = dbModel.id, type = dbModel.type, title = dbModel.title, imageVector = dbModel.imageVector)
    }
    fun entityToDbModel(entity: DayAttribute):DayAttributesDbModel {
        return DayAttributesDbModel(id = entity.id, type = entity.type, title = entity.title, imageVector = entity.imageVector)
    }
}